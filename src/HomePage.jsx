...
function HomePage({searchTerm,setSearchTerm,term}) {
  ...
  useEffect(()=>{
    ...
    return (
<Fragment>
    <Navbar posts={posts} sort={sort} setSort={setSort} term={term} searchTerm={searchTerm} setSearchTerm={setSearchTerm}></Navbar>
    <div className="App">
      ...
      {loading&&filteredItems.map((post,index)=>{
        return(
       <div key={post.id}>
          <Card className="card">
        <CardContent>
          <Typography style={{display:'flex',alignItems:'center'}} gutterBottom variant="h5" component="div">
            <BorderColorIcon/>{searchTerm===''?post.title: highlightSearchText(post.title, searchTerm)}
          </Typography>
      ...
      <CardActions style={{display:'flex',justifyContent:'space-between'}}>
        ...
        <IconButton onClick={()=>{setDeleteAlert(true);setSelectedIndex(index)}}>
        ...
        <DialogContent>
          <DialogContentText>
            <Typography style={{fontSize:'1.25rem'}}>
              Are You Surely want to delete this Post?
            </Typography>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
        <Button variant="outlined" onClick={()=>deletePost(selectedIndex)}>Ok</Button>
        <Button variant="contained" onClick={()=>{setDeleteAlert(false);setComments([])}}>Cancel</Button>
        </DialogActions>
      </Dialog>
    </Fragment>
     );
